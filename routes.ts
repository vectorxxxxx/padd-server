...
import * as balanceTx from '../padd-ui/lib/balanceTx';
import adminHelper from '../padd-ui/lib/firebaseAdmin';
...
// Ensure the padd-ui firebase admin initializer runs and provide a local helper
function getAdminDb() {
  try {
    return adminHelper.database();
  } catch (e) {
    console.error('[SERVER] getAdminDb helper failed', e);
    throw e;
  }
}
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // (GMGN proxy removed) - frontend will use direct embed again.
  ...
  app.get("/api/wallet/balance/:publicKey", isAuthenticated, async (req: any, res) => {
    try {
      ...
      // Prevent caching/conditional responses so clients always receive a JSON body.
      // Set no-cache headers and a unique ETag to avoid matching If-None-Match.
  ...
  // Return a user's stored wallet balance (from Firebase / storage) by user id (no session required)
  app.get('/api/wallet/balance/user/:userId', async (req: any, res) => {
    try {
      const { userId } = req.params;
      if (!userId) return res.status(400).json({ error: 'userId required' });

      // Find the user's primary wallet and return its stored balance
      const wallet = await storage.getUserWallet(String(userId));
      if (!wallet) return res.json({ balance: 0 });
      const bal = Number(wallet.balance) || 0;
      // Prevent caching/conditional responses so clients always receive a JSON body.
      // Set no-cache headers and a unique ETag to avoid matching If-None-Match.
      const etag2 = String(Date.now());
      res.set({
        'Cache-Control': 'no-store, no-cache, must-revalidate, proxy-revalidate',
        Pragma: 'no-cache',
        Expires: '0',
      ...
      return res.status(200).send(JSON.stringify({ balance: bal }));
    } catch (err) {
      console.error('/api/wallet/balance/user error', err);
  ...
  app.post("/api/engine/vaults/create", async (req: any, res) => {
    try {
      console.log('[API][ENGINE][VAULT_CREATE] incoming', { path: req.path });

      // Determine caller identity: prefer session (passport), then Authorization Bearer idToken, then session-based wallet user
      let uid: string | undefined | null = undefined;
      if (req.isAuthenticated && typeof req.isAuthenticated === 'function' && req.isAuthenticated()) {
        uid = req.user?.claims?.sub || req.user?.id;
        console.log('[API][ENGINE][VAULT_CREATE] session auth detected', { uid });
      } else {
        const auth = (req.headers?.authorization || '') as string;
        if (auth.startsWith('Bearer ')) {
          const idToken = auth.replace('Bearer ', '');
          try {
            ...
            const decoded = await admin.auth().verifyIdToken(idToken);
            uid = decoded?.uid;
            console.log('[API][ENGINE][VAULT_CREATE] firebase token verified', { uid });
          } catch (e) {
            console.error('[API][ENGINE][VAULT_CREATE] Firebase token verification failed', e);
            uid = null;
          }
        }
      }

      // Fallback to session user set by /api/auth/wallet-connect
      if (!uid && req.session && req.session.user) {
        uid = req.session.user.id || req.session.user?.publicKey || null;
        console.log('[API][ENGINE][VAULT_CREATE] wallet-session detected', { uid });
      }

      // Allow client to provide `userId` (wallet public key stored as user id in Firebase)
      // This is used when no session or idToken is available (wallet-only flow).
      if (!uid && req.body && req.body.userId) {
        uid = String(req.body.userId);
        console.log('[API][ENGINE][VAULT_CREATE] using userId from body as uid', { uid });
      }

      if (!uid) return res.status(401).json({ success: false, error: 'unauthenticated' });

      const { mint, params, name, currency } = req.body || {};
      if (!mint || !params) return res.status(400).json({ success: false, error: 'mint and params required' });

      // If the client provided an initial SOL amount as part of the create request,
      // attempt to deduct it from the user's stored off-chain balance atomically
      const initialSol = Number(req.body?.initialAmount ?? req.body?.amountSol ?? 0) || 0;
      if (initialSol > 0) {
        try {
          const db = getAdminDb();
          const balanceRef = db.ref(`/users/${uid}/balance`);
          // attempt to atomically subtract initialSol
          const txRes = await balanceTx.transactionWithReadGuard(balanceRef, (cur) => {
            if (cur < initialSol) return undefined;
            return cur - initialSol;
          }, { attempts: 6, backoffMs: 50, tag: 'create_deduct' });

          if (!txRes || !(txRes as any).committed) {
            console.warn('[API][ENGINE][VAULT_CREATE] insufficient balance for initial create deposit', { uid, initialSol });
            return res.status(400).json({ success: false, error: 'insufficient_balance' });
          }
          console.log('[API][ENGINE][VAULT_CREATE] deducted initial balance for create', { uid, initialSol });
        } catch (deductErr: any) {
          console.error('[API][ENGINE][VAULT_CREATE] failed to deduct initial balance', deductErr);
      ...
      if (!engine || typeof engine.createVault !== 'function') {
        ...
        if (initialSol > 0) {
          try {
            const db = getAdminDb();
            const balanceRef = db.ref(`/users/${uid}/balance`);
            await balanceTx.transactionWithReadGuard(balanceRef, (cur) => (cur + initialSol), { attempts: 6, backoffMs: 50, tag: 'create_refund' });
            console.log('[API][ENGINE][VAULT_CREATE] refunded initial deduction after missing engine', { uid, initialSol });
          } catch (refundErr) {
            console.error('[API][ENGINE][VAULT_CREATE] failed to refund after missing engine', refundErr);
          }
        }
        return res.status(500).json({ success: false, error: 'engine.createVault unavailable' });
      }

      try {
        const result = await engine.createVault(String(mint), String(uid), params, name, currency);
        if (!result || !result.ok) {
          ...
          if (initialSol > 0) {
            try {
              ...
              const balanceRef = db.ref(`/users/${uid}/balance`);
              await balanceTx.transactionWithReadGuard(balanceRef, (cur) => (cur + initialSol), { attempts: 6, backoffMs: 50, tag: 'create_refund' });
              console.log('[API][ENGINE][VAULT_CREATE] refunded initial deduction after create failure', { uid, initialSol });
            } catch (refundErr) {
              console.error('[API][ENGINE][VAULT_CREATE] failed to refund after create failure', refundErr);
            }
          }
        ...
        if (initialSol > 0) {
          try {
            ...
            const balanceRef = db.ref(`/users/${uid}/balance`);
            await balanceTx.transactionWithReadGuard(balanceRef, (cur) => (cur + initialSol), { attempts: 6, backoffMs: 50, tag: 'create_refund' });
            console.log('[API][ENGINE][VAULT_CREATE] refunded initial deduction after create exception', { uid, initialSol });
          } catch (refundErr) {
            console.error('[API][ENGINE][VAULT_CREATE] failed to refund after create exception', refundErr);
          }
        }
        throw err;
      }
    } catch (err: any) {
      console.error('/api/engine/vaults/create error', err);
      return res.status(500).json({ success: false, error: err?.message || String(err) });
    }
  });

  // Deposit into a vault (creator deposit or contributor deposit) - accepts same auth methods
  app.post('/api/engine/vaults/deposit', async (req: any, res) => {
    try {
      console.log('[API][ENGINE][VAULT_DEPOSIT] incoming', { path: req.path });
      ...
      if (req.isAuthenticated && typeof req.isAuthenticated === 'function' && req.isAuthenticated()) {
        ...
        if (auth.startsWith('Bearer ')) {
          ...
          try {
            ...
            console.error('[API][ENGINE][VAULT_DEPOSIT] Firebase token verification failed', e);
            uid = null;
          }
        }
      }
      if (!uid && req.session && req.session.user) {
        uid = req.session.user.id || req.session.user?.publicKey || null;
        console.log('[API][ENGINE][VAULT_DEPOSIT] wallet-session detected', { uid });
      }

      // Allow client to provide `userId` in deposit requests when no session or idToken is present
      if (!uid && req.body && req.body.userId) {
        uid = String(req.body.userId);
        console.log('[API][ENGINE][VAULT_DEPOSIT] using userId from body as uid', { uid });
      }
      ...
      try {
        ...
        try {
          const readSnap = await balanceRef.get();
          const readVal = readSnap && readSnap.exists() ? readSnap.val() : null;
          console.log('[API][ENGINE][VAULT_DEPOSIT] pre-deduct balance', { uid, readVal });
        } catch (e) {
          console.warn('[API][ENGINE][VAULT_DEPOSIT] failed to read balance before deduct', e);
        }

        const txRes = await balanceTx.transactionWithReadGuard(balanceRef, (cur) => {
          if (cur < amt) return undefined;
          return cur - amt;
        }, { attempts: 6, backoffMs: 50, tag: 'deposit_deduct' });

        console.log('[API][ENGINE][VAULT_DEPOSIT] txRes after transactionWithReadGuard', { uid, txRes });

        if (!txRes || !(txRes as any).committed) {
          console.warn('[API][ENGINE][VAULT_DEPOSIT] insufficient balance for deposit', { uid, amt, txRes });
          return res.status(400).json({ success: false, error: 'insufficient_balance' });
        }

        try {
          const afterSnap = await balanceRef.get();
          const afterVal = afterSnap && afterSnap.exists() ? afterSnap.val() : null;
          console.log('[API][ENGINE][VAULT_DEPOSIT] post-deduct balance read', { uid, before: undefined, afterVal });
        } catch (e) {
          console.warn('[API][ENGINE][VAULT_DEPOSIT] failed to read post-deduct balance', e);
        }

        console.log('[API][ENGINE][VAULT_DEPOSIT] deducted user balance', { uid, amt });
      } catch (deductErr: any) {
        console.error('[API][ENGINE][VAULT_DEPOSIT] failed to deduct user balance', deductErr);
        return res.status(500).json({ success: false, error: deductErr?.message || String(deductErr) });
      }
      ...
      try {
        // Try creator deposit first; if it fails with 'not creator', fall back to contributor deposit
        const creatorRes = await engine.creatorDeposit(String(mint), amt, String(uid));
        return res.json({ success: true, result: creatorRes });
      } catch (e: any) {
        // If creatorDeposit indicates requester is not the creator, fall back to contributor deposit
        if (String(e?.message || '').toLowerCase().includes('not creator')) {
          try {
            const contrib = await engine.contributorDeposit(String(mint), amt, String(uid), feeKeepPct ? { feeKeepPct } : undefined);
            return res.json({ success: true, result: contrib });
          } catch (ce: any) {
            console.error('[API][ENGINE][VAULT_DEPOSIT] contributorDeposit failed', ce);
            // Attempt to refund deducted balance since deposit did not complete
            try {
              const db = getAdminDb();
              const balanceRef = db.ref(`/users/${uid}/balance`);
              await balanceTx.transactionWithReadGuard(balanceRef, (cur) => (cur + amt), { attempts: 6, backoffMs: 50, tag: 'deposit_refund' });
              console.log('[API][ENGINE][VAULT_DEPOSIT] refunded user balance after failed contributorDeposit', { uid, amt });
            } catch (refundErr) {
              console.error('[API][ENGINE][VAULT_DEPOSIT] failed to refund after contributorDeposit failure', refundErr);
            }
            return res.status(400).json({ success: false, error: ce?.message || String(ce) });
          }
        }
        console.error('[API][ENGINE][VAULT_DEPOSIT] creatorDeposit error', e);
        // deposit failed after deduction; attempt to refund
        try {
          const db = getAdminDb();
          const balanceRef = db.ref(`/users/${uid}/balance`);
          await balanceTx.transactionWithReadGuard(balanceRef, (cur) => (cur + amt), { attempts: 6, backoffMs: 50, tag: 'deposit_refund' });
          console.log('[API][ENGINE][VAULT_DEPOSIT] refunded user balance after creatorDeposit failure', { uid, amt });
        } catch (refundErr) {
          console.error('[API][ENGINE][VAULT_DEPOSIT] failed to refund after creatorDeposit failure', refundErr);
        }
        return res.status(400).json({ success: false, error: e?.message || String(e) });
      }
    } catch (err: any) {
      console.error('/api/engine/vaults/deposit error', err);
      return res.status(500).json({ success: false, error: err?.message || String(err) });
    }
  });
